...
public class NJ_Oxford_Parser_Temp {
	
	PDFManager pdfmanager;
	ArrayList<Page> result;
	
	public NJ_Oxford_Parser_Temp(File file) throws IOException {
	...
	public void parse(int pageNum) throws IOException {
		...
		try {
			...
			while (index < tokens.length) {
				...
				while (index < tokens.length && !tokens[index].equals("Oxford®") && !tokens[index].equals("PPO")
				...
				//Plan name
				while (index < tokens.length && !tokens[index].equals("Garden") && !tokens[index].equals("Freedom") 
				...
				//Network/Access
				sb.setLength(0);
				while (index < tokens.length && !tokens[index].equals("Gated")) {
					sb.append(tokens[index] + " ");
					index++;
				}
				sb.append(tokens[index]);
	...
	//			}
				...
				try {
					...
					sb.append(tokens[index] + " ");
					if (tokens[index + 5].equals("year")) {
						for (int i = 1; i < 6; i++) {
							sb.append(tokens[index + 1] + " ");
							index++;
						}
					} else if (tokens[index + 8].equals("year")) {
						for (int i = 1; i < 9; i++) {
							sb.append(tokens[index + 1] + " ");
							index++;
						}
					} else if (tokens[index + 7].equals("year")) {
						for (int i = 1; i < 8; i++) {
							sb.append(tokens[index + 1] + " ");
							index++;
						}
				...
				index++;
				System.out.println("Outpatient services freestanding: " + outpatient_services);
				...
				System.out.println("Outpatient services hospital: " + outpatient_services_hospital );
				index++;
				...
				System.out.println("Laboratory services: " + laboratory_services);
				index++;
				...
				System.out.println("Major Diagnostic Freestanding: " + major_diagnostic_freestanding);
				index++;
				...
				try {
					...
					while (!tokens[index].equals("Oxford®")) {
						sb.append(tokens[index] + " ");
						index++;
					}
				}
