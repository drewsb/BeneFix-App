...
public class BaseRateRetriever implements Parser{
	...
	public ArrayList<Page> parse(File file, String filename) throws IOException {
		...
		while(!tokens[index-1].contains("4Q")){
			index++;
		}
		
		while(!tokens[index].equals("Quarterly")){
			String product_name = "";
			...
			while(!Formatter.isDecimal(tokens[index])){
