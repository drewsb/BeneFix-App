 ...
 */
public class Aetna_ExcelWriter {
	 ...
	 */
	public static void populateExcel(Aetna_Page[] pages, String filename) throws IOException {
        XSSFWorkbook workbook = new XSSFWorkbook();
        XSSFSheet sheet = workbook.createSheet("BenefixData");
        String outputName;
         
        //Template data given by Benefix. 
        String[] templateData = {"start_date", "end_date",	"product_name",
        ...
        for(Aetna_Page p : pages){
            ...
            cell = row.createCell(colCount++);
            cell.setCellValue((String) p.rating_area);
            cell = row.createCell(colCount++);
            cell.setCellValue(p.age_dict.get("0-20"));
            cell = row.createCell(colCount++);
            cell.setCellValue(p.age_dict.get("0-20"));
            for(int i = 0; i < 43; i++){
            	cell = row.createCell(colCount++);
            	String index = String.format("%d", i+21);
                cell.setCellValue(p.age_dict.get(index));
            }
            cell = row.createCell(colCount++);
            cell.setCellValue(p.age_dict.get("64+"));
            cell = row.createCell(colCount++);
            cell.setCellValue(p.age_dict.get("64+"));
        }
        if(!filename.contains("Aetna")){
            outputName = String.format("Aetna_%s_data.xlsx", filename);
        }
        else{
